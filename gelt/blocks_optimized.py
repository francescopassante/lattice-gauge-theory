...
(see notes/architecture.html §13.1):
...
is byte-for-byte the original implementation — see gelt/blocks.py for the
original docstrings and pipeline explanation; the same explanatory comments
are mirrored here on the shared code.
"""
...
class GEMHSA(nn.Module):
    ...
    def _attend(self, Q, K, V, T):
        ...
        # 2. Adjoint transport: T · X · T†.
        # Optimisation vs blocks.py: instead of broadcasting T over (H, d_qkv)
        # in a tiny-matmul batch, ``_transport_folded`` collapses (H, d_qkv)
        # into the column of the right-multiplicand. T is computed once per
        # layer (shared between K and V) — its dagger too.
        ...
        # mathematically identical to the original "matmul then weight then
